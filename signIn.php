<?php
	...
	if(isset($_POST['login'])){
		if ((!empty($_POST['id'])) && (!empty($_POST['password']))){
			...
			$query = "SELECT * FROM engineer_users WHERE username = '$username' AND password = '$password' limit 1";
			
			$result = mysqli_query($db,$query);
			
			if ((strtolower($_POST['id']) == 'admin') && ($_POST['password'] == 'admin123')) {
    ...
    <form method="post" action="signIn.php">
		<div class="site-section">
			<div class="container">
							...
							if(isset($_POST['login'])){
								if ((!empty($_POST['id'])) && (!empty($_POST['password']))){
									...
									if ((strtolower($_POST['id']) == 'admin') && ($_POST['password'] == 'admin123')) {
										...
										echo '<p style="color: red;">Failed to login! Incorrect username or password. Please try again.</p>';			
									}
								} else {
									echo '<p style="color:red;">You did not key in ID or password!</p>';
								}
							}
						?>
					</div>
				...
				<div class="row">
					<div class="col-12">
						<input type="submit" name="login" value="Login" class="btn btn-primary rounded-0 px-3 px-5" />
					</div>
				</div>
				...
				<div class = "row">
					<div class="col-6">
						...
						<p><br />Can't remember your ID and/or password? <a href="retrieve.php">Click here!</a></p>
					</div>
				</div>
			</div>
		</div>
